...
/** Every form in the checklist, drawn to the four sentences above. The pilots
 *  (chair, dresser, sofa, lamp) were walked with the owner first; the rest were
 *  filled to them, and await the same walk. */
export const FARMHOUSE_ART: Partial<Record<FurnitureId, PieceArt>> = {
  ...
  // A worn bordered rug — the piece in four of the five photographs. Rust ground, ochre guard stripes, a diamond lattice in blue and cream, and a fringe on the short ends.
  arearug: {
    mirrorW: true,
    s: {
      rows: [
        ...
        "..koooooooooooooooooooooooooooooooooooooooook...",
        "..koooooooooooooooooooooooooooooooooooooooook...",
        "..koooyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyoook...",
        "..koooycccccccccccccccccccccccccccccccccyoook...",
        "..koooycccccccccccccccccccccccccccccccccyoook...",
        "..koooyccccycccccccccccycccccccccccyccccyoook...",
        "..koooycccyyycccccccccyyycccccccccyyycccyoook...",
        "..koooyccyybyycccccccyybyycccccccyybyyccyoook...",
        "..koooycccyyycccccccccyyycccccccccyyycccyoook...",
        "..koooyccccycccccccccccycccccccccccyccccyoook...",
        "..koooycccccccccccccccccccccccccccccccccyoook...",
        "..koooycccccccccccccccccccccccccccccccccyoook...",
      ...
      palette: { k: INK, o: "#9c4a3c", y: "#c9a25c", b: "#3f5a72", c: "#e6dcc4" },
    },
  },

  // THE CHAIR'S SPINDLES, in a headboard — the one place a set gets to repeat itself and be right, because a farmhouse bed and a farmhouse chair came out of the same workshop.
  bed: {
    mirrorW: true,
    s: {
      rows: [
    ...
    e: {
      rows: [
        ...
        "kck.kooooookCCCCCCCCCCCCCCCCkcc.",
        "kck.kooooookCCCCCCCCCCCCCCCCkcc.",
        "kck.kooooookCCCCCCCCCCCCCCCCkcc.",
        "kck.kooooookCCCCCCCCCCCCCCCCkcc.",
        "kck.kooooookCCCCCCCCCCCCCCCCkcc.",
  ...
  // The table's top on the chair's legs, and nothing else. A bench that grew a back would be a settle, and a settle is a different form.
  bench: {
    mirrorW: true,
    s: {
      rows: [
  ...
  // The table again, cut down. Every farmhouse coffee table in the references is a dining table that lost its legs.
  coffeetable: {
    mirrorW: true,
    s: {
      rows: [
        ...
        ".kkkkkkkkkkkkkkkkkkkkkkkkkkkkkk.",
        ".kttttttttttttttttttttttttttttk.",
        ".ktttttttttttttsstttttttttttttk.",
        ".kttttttttttttttttttttttttttttk.",
        ".kcccccccccccccccccccccccccccck.",
        ".kssssssssssssssssssssssssssssk.",
        ".kkkkkkkkkkkkkkkkkkkkkkkkkkkkkk.",
  ...
  // A panelled door and the set's bun foot. It joins like core's, so a run of them is a kitchen.
  counter: {
    mirrorW: true,
    s: {
      rows: [
  ...
  // Rule 3, in the smallest piece: a soft thing sits down. No lift, no legs, and the corners round off before the floor.
  cushion: {
    mirrorW: true,
    s: {
      rows: [
  ...
  // The table's top without the seam — a desk is one board — on the same legs.
  desk: {
    mirrorW: true,
    s: {
      rows: [
  ...
  // The floor lamp's cone, shortened, and the same SHADE_CLOTH. A lamp is a lamp at either height.
  desklamp: {
    mirrorW: true,
    s: {
      rows: [
  ...
  // The single bed, twice as wide, with the spindles carried across rather than doubled up.
  doublebed: {
    mirrorW: true,
    s: {
      rows: [
  ...
  // An open hearth under a thick mantel slab, with the fire in a brick throat. Moderne's is a cone in the middle of the room; this one is a wall.
  fireplace: {
    mirrorW: true,
    s: {
      rows: [
        ...
        ".kcccccckeeeeeeeeeeeeeekcccccck.",
        ".kcccccckeeeeeeeeeeeeeekcccccck.",
        ".kcccccckeeeeeffffeeeeekcccccck.",
        ".kcccccckeeeeeffffeeeeekcccccck.",
        ".kcccccckeeeeooooooeeeekcccccck.",
        ".kcccccckeeeeooooooeeeekcccccck.",
        ".kcccccckggggggggggggggkcccccck.",
        ".kcccccccccccccccccccccccccccck.",
        ".kcccccccccccccccccccccccccccck.",
        ".kkkkkkkkkkkkkkkkkkkkkkkkkkkkkk.",
        ".kssssssssssssssssssssssssssssk.",
  ...
  // The one anachronism the set has to carry, so it is drawn as a larder — a tall panelled cupboard with one knob, which is what a farmhouse kitchen would have had where the fridge now stands.
  fridge: {
    mirrorW: true,
    s: {
      rows: [
  ...
  houseplant: {
    ...
    s: {
      ...
      palette: { k: INK, g: "#5d9a37" },
    },
  },

  // The basin's apron at counter width.
  kitchensink: {
    mirrorW: true,
    s: {
      rows: [
  ...
  // A landscape, which is what is on every wall in the references.
  painting: {
    mirrorW: true,
    s: {
      rows: [
      ...
      palette: { k: INK, w: "#efe6d2", g: "#7d9a6a", h: "#b06a3c", d: "rgba(0,0,0,0.20)" },
    },
  },

  // Plank shelves between two uprights, on bun feet — the open half of the reference's hutch. The boards are `s` so they read as underside rather than as more carcass.
  shelf: {
    mirrorW: true,
    s: {
      rows: [
  ...
  // An APRON FRONT — the bowl's face IS the cabinet's face, which is the farmhouse sink and nothing else's.
  sink: {
    mirrorW: true,
    s: {
      rows: [
  ...
  // A chair with the back taken off, which is what a stool is. Same seat, same leg.
  stool: {
    mirrorW: true,
    s: {
      rows: [
  ...
  // A black range: iron top, two pairs of rings, and a glass door under a panelled front.
  stove: {
    mirrorW: true,
    s: {
      rows: [
  ...
  // The plank top, with its SEAM down the middle — rule 2's clearest case, and the reason this form joins: three of these in a row is the long dining table (DESIGN §laid by the yard), not a wider form.
  table: {
    mirrorW: true,
    s: {
      rows: [
  ...
  // Core's shape, farmhouse's cistern: a tall square tank rather than a low one, because the low one is Moderne's whole idea.
  toilet: {
    mirrorW: true,
    s: {
      rows: [
  ...
  // THE CLAWFOOT, the one thing kept from the fourth photograph (owner, 20 Aug). Rolled rim, and four feet that step outward as they reach the floor — the only splay in the set, and it belongs to the piece that historically had it.
  tub: {
    mirrorW: true,
    s: {
      rows: [
