...
describe("crop growth", () => {
  it("does not grow until watered", () => {
    const w = freshWorld();
    const t0 = 1_000_000;
    expect(plant(w, 4, 17, "carrot", t0)).toBe(true);
    // A full day passes with no water: still stage 0.
    updateCrop(w, 4, 17, t0 + 24 * HOUR);
    expect(w.crops[tileKey(4, 17)].stage).toBe(0);
    expect(w.crops[tileKey(4, 17)].growthMs).toBe(0);
  });

  it("advances stages only while watered, and stalls when dry", () => {
    const w = freshWorld();
    const t0 = 1_000_000;
    plant(w, 4, 17, "carrot", t0);
    water(w, 4, 17, t0); // wet until t0 + WATER_DURATION_MS
    // Stage 0 needs 2h watered; after 2h it should reach stage 1.
    updateCrop(w, 4, 17, t0 + 2 * HOUR);
    expect(w.crops[tileKey(4, 17)].stage).toBe(1);
    // Let the soil dry (past WATER_DURATION), then a long absence adds no growth.
    const dry = t0 + WATER_DURATION_MS;
    updateCrop(w, 4, 17, dry);
    const stageAtDry = w.crops[tileKey(4, 17)].stage;
    updateCrop(w, 4, 17, dry + 100 * HOUR);
    expect(w.crops[tileKey(4, 17)].stage).toBe(stageAtDry);
  });
  ...
  it("reaches ripe with enough watered time and can be harvested", () => {
    ...
    for (let h = 1; h <= totalHours; h++) {
      now = t0 + h * HOUR;
      water(w, 4, 17, now); // keep it soaked
    }
    updateCrop(w, 4, 17, now);
    expect(isRipe(w, 4, 17)).toBe(true);
    const seedBefore = count(w.inventory, "seed");
    expect(w.crops[tileKey(4, 17)].stage).toBe(ripeStage(def));
    ...
    const yielded = harvest(w, 4, 17, now);
    ...
    expect(w.crops[tileKey(4, 17)]).toBeUndefined();
  });
  ...
  it("away-sim is identical whether stepped or jumped", () => {
    ...
    const a = freshWorld();
    const b = freshWorld();
    plant(a, 4, 17, "carrot", t0);
    plant(b, 4, 17, "carrot", t0);
    water(a, 4, 17, t0);
    water(b, 4, 17, t0);
    // Only 5h elapse — under the 22h water window, so no drying involved.
    updateCrop(a, 4, 17, t0 + 5 * HOUR);
    for (let h = 1; h <= 5; h++) updateCrop(b, 4, 17, t0 + h * HOUR);
    expect(a.crops[tileKey(4, 17)].stage).toBe(b.crops[tileKey(4, 17)].stage);
    expect(a.crops[tileKey(4, 17)].growthMs).toBe(b.crops[tileKey(4, 17)].growthMs);
  });
  ...
  it("watering sets the soil wet and it dries back after the window", () => {
    ...
    const t0 = 1_000_000;
    plant(w, 4, 17, "carrot", t0);
    expect(tileAt(w, 4, 17)).toBe(FARMLAND);
    water(w, 4, 17, t0);
    expect(tileAt(w, 4, 17)).toBe(FARMLAND_WET);
    updateCrop(w, 4, 17, t0 + WATER_DURATION_MS + HOUR);
    expect(tileAt(w, 4, 17)).toBe(FARMLAND);
  });
});
